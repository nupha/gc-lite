...
use gc_lite::{GcHeap, GcRef, GcTracable, GcTracer};
...
fn benchmark_object_sizes() {
    ...
    for &size in &sizes {
        ...
        let partition = context.create_partition("benchmark".to_string(), Some(1024 * 1024 * 10)); // 10MB
        ...
        let freed = context.collect_garbage(partition);
...
fn benchmark_complex_graphs() {
    ...
    for &size in &sizes {
        ...
        let mut context = GcHeap::new();
        let partition = context.create_partition("complex".to_string(), Some(1024 * 1024 * 10));
...
fn benchmark_memory_efficiency() {
    ...
    let mut context = GcHeap::new();
    let partition = context.create_partition("efficiency".to_string(), Some(1024 * 1024)); // 1MB
...
fn benchmark_auto_gc_threshold() {
    ...
    let mut context = GcHeap::new();
    let partition = context.create_partition("auto_gc".to_string(), Some(2048)); // 2KB limit
...
unsafe impl GcTracable for SimpleNode {
    fn trace(&self, _tracer: &mut GcTracer) {
        // Vec<u8> doesn't contain GC references, no need to trace
    }
}
